...
*/
function buildPathAliasScopes(paths) {
	const scopes = /* @__PURE__ */ new Set();
	if (paths !== void 0) for (const alias of Object.keys(paths)) {
		const scope = extractScope(alias);
		if (scope !== null) scopes.add(scope);
	}
	return scopes;
}
function buildScopeConfig(context) {
	const { configFilePath, paths, typeRoots } = getParserServices(context).program.getCompilerOptions();
	return {
		allowedScopes: buildAllowedScopes(typeRoots, typeof configFilePath === "string" ? dirname(configFilePath) : context.cwd),
...
*/
function checkModuleSource(context, config, node, source) {
	if (!isScopedPackage(source)) return;
	const scope = extractScope(source);
	if (scope === null) return;
	if (config.pathAliasScopes.has(scope)) return;
...
function create$6(context) {
	...
	return {
		...
		ExportNamedDeclaration(node) {
			if (node.source?.value !== void 0) checkModuleSource(context, config, node, node.source.value);
		},
		ImportDeclaration(node) {
			checkModuleSource(context, config, node, node.source.value);
		},
		ImportExpression(node) {
			if (node.source.type === AST_NODE_TYPES.Literal && typeof node.source.value === "string") checkModuleSource(context, config, node, node.source.value);
		}
	};
}
/**
